<section>
    ...
    <script>
        function previewPhoto(event) {
            console.log('Fonction previewPhoto appelée');
            const file = event.target.files[0];
            const preview = document.getElementById('photo-preview');
            
            if (!preview) {
                console.error('Élément photo-preview introuvable');
                return;
            }
            ...
            if (file) {
                ...
                reader.onload = function(e) {
                    ...
                    if (preview.tagName.toLowerCase() === 'div') {
                        ...
                        img.alt = 'Aperçu';
                        img.className = 'w-16 h-16 rounded-full object-cover border-2 border-gray-300 dark:border-gray-600';
                        preview.parentNode.replaceChild(img, preview);
                        img.id = 'photo-preview'; // Réattribuer l'ID
                    } else {
                        // Si c'est déjà une image, on met juste à jour la source
